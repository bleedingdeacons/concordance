...
/**
 * Build Script for Concordance WordPress Plugin
 *
...
class PluginBuilder
{
    ...
    private string $pluginName = 'concordance';
    ...
    private array $productionExcludes = [
        ...
        // Composer files (lock not needed after autoload is generated; keep composer.json for WP plugin checker)
            'composer.lock',
    ...
    {
        $mainFile = $this->pluginDir . DIRECTORY_SEPARATOR . 'concordance.php';
        if (file_exists($mainFile)) {
            $content = file_get_contents($mainFile);
            if (preg_match('/Version:\s*([0-9.]+\s*\w*)/', $content, $matches)) {
                return trim($matches[1]);
            }
            if (preg_match("/define\s*\(\s*'CONCORDANCE_VERSION'\s*,\s*'([^']+)'/", $content, $matches)) {
                return $matches[1];
            }
        }
        return '0.0.1';
...
Concordance WordPress Plugin Build Script
==========================================
Platform: {$platform}
...
Files Excluded (Production):
  ...
  - Build configuration (composer.lock, package.json, etc.)
...
Note:
  Concordance is a standalone WordPress plugin. No external plugin
  dependencies are required — just install, activate, and configure
  your API key under Settings → Concordance API.
